...
def visualize_voxel_grid(
    ...
    points = np.column_stack((x[occupied[0]], y[occupied[1]], z[occupied[2]]))
    ...
    plotter.add_mesh(
        pv.Sphere(radius=4, center=transformed_axis), color="blue", label="Axis Point"
    )
    plotter.add_mesh(
        pv.Sphere(radius=4, center=transformed_base), color="red", label="Base Point"
    )
    plotter.show_axes()
    plotter.show_grid()
    plotter.show()
...
if __name__ == "__main__":
    ...
    if os.path.exists("points.npy"):
        ...
        residues = filter_residues_parallel(
            ribosome_entities(RCSB_ID, "R"),
            base_point,
            axis_point,
            radius,
            height,
        ...
        points = np.array(
            [atom.get_coord() for residue in residues for atom in residue.child_list]
        )
