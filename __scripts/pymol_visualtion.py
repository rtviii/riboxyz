...
def get_markerspath(struct: str):
    struct = struct.upper()
    _path = os.path.join(
        RIBETL_DATA, "PTC_MARKERS_RAW", f"{struct}_PTC_MARKERS_RAW.json"
    )

    return _path


def create_marker_at_atom(
    selection_name: str,
    posn: List[float],
    color_: str = "red",
    repr="spheres",
...
def pseudoatom_ptc(rcsb_id: str):
    ...
    profile = ops.profile()
    ptc_path = ops.paths.ptc

    if not os.path.isfile(ptc_path):
        reslist, auth_asym_id = ptc_resdiues_get(
            ops.biopython_structure(), profile.rnas, 0
        )
...
def by_chain(pdbid: str):
    ...
    for protein in list(
        sorted(profile["proteins"], key=lambda d: d["nomenclature"][0])
    ):
        if len(protein["nomenclature"]) > 0:
            nomclass = protein["nomenclature"][0]
        else:
            nomclass = random.choice(
                [
                    *list(colormap__LSU_Proteins.keys()),
                    *list(colormap__SSU_Proteins.keys()),
                ]
            )
...
def ray_picture(pdbid: str):
    ...
    cmd.png(
        f"/home/rtviii/dev/riboxyz/rays/{pdbid}.png",
        ray=1,
        width=400,
        height=400,
        dpi=300,
...
def create_centerline(rcsb_id: str):
    ...
    cmd.select(
        name="_surf25", selection="{} within 25 of {}".format(rcsb_id, object_name)
    )
    cmd.create("surf25", "_surf25")

    cmd.select(
        name="_surf15", selection="{} within 15 of {}".format(rcsb_id, object_name)
    )
