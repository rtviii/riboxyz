...
def classify_ligand(ligand):
    ...
    flattened_fields = [
        ...
        (ligand.get("direct_parent") or {}).get(
            "name", ""
        ),  # Safeguard for 'direct_parent'
    ]

    # Add intermediate nodes (if present)
    flattened_fields.extend(
        [(node or {}).get("name", "") for node in ligand.get("intermediate_nodes", [])]
    )
    ...
    flattened_fields.extend(
        [term.split(" (")[0] for term in ligand.get("predicted_chebi_terms", [])]
    )
    ...
    for category in categories:
        if any(
            category.lower() in field.lower() for field in flattened_fields if field
        ):  # Skip empty fields
