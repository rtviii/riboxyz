...
def apply_poisson_reconstruction( surf_estimated_ptcloud_path:str, output_path:str):
    import plyfile
    # command   = [ POISSON_RECON_BIN, "--in", surface_with_normals_path(rcsb_id), "--out", poisson_recon_path, "--depth", "6", "--pointWeight", "3", ]
    command   = [ POISSON_RECON_BIN, "--in", surf_estimated_ptcloud_path, "--out", output_path, "--depth", "6", "--pointWeight", "3", ]
...
def estimate_normals(convex_hull_surface_pts: np.ndarray, output_path:str):
