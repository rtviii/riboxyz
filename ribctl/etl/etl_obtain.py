...
def asset_routines(
    rcsb_id: str, assetlist: list[AssetClass], overwrite: bool = False, reclassify=False
) -> list[Coroutine]:
    ...
    if AssetClass.profile in assetlist:
        coroutines.append(ETLCollector(rcsb_id).generate_profile(overwrite, reclassify))
