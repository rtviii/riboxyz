...
class PolymersNode:
        ...
        for polymer_dict in self.rcsb_data_polymers:
            ...
            for poly in polys:
                match poly.entity_poly_polymer_type:
                    ...
                    case "RNA":
                        rna = RNA.model_validate(
                            ({**polymer_dict, **poly.model_dump()})
                        )
        ...
        if not os.path.exists(RA.paths.classification_report):
            ...
            with open(RA.paths.classification_report, "r") as infile:
                ...
                reported_classes = {
                    ...
                    for (auth_asym_id, polymer_class_hits) in full_report.items()
                }
    ...
    #                 reshaped_polymeric_factors.extend(self.poly_reshape_to_factor(poly))
    ...
    #                 ""
    #                 if rrna_polymer_obj["rcsb_polymer_entity"]["pdbx_description"]
    #                 == None
    ...
    def raw_to_polymer(
        self, polymer_entity: dict, parent_rcsb_id: str
    ) -> list[Polymer]:

        host_organisms: list[Any] | None = polymer_entity["rcsb_entity_host_organism"]
        source_organisms: list[Any] | None = polymer_entity[
            "rcsb_entity_source_organism"
        ]
        ...
        return [
            Polymer(
                ...
                asym_ids=polymer_entity["rcsb_polymer_entity_container_identifiers"][
                    "asym_ids"
                ],
                auth_asym_id=auth_asym_id,
                ...
                rcsb_pdbx_description=(
                    ""
                    if polymer_entity["rcsb_polymer_entity"]["pdbx_description"] == None
                    else polymer_entity["rcsb_polymer_entity"]["pdbx_description"]
                ),
                entity_poly_strand_id=polymer_entity["entity_poly"]["pdbx_strand_id"],
                entity_poly_seq_one_letter_code=polymer_entity["entity_poly"][
                    "pdbx_seq_one_letter_code"
                ],
                entity_poly_seq_one_letter_code_can=polymer_entity["entity_poly"][
                    "pdbx_seq_one_letter_code_can"
                ],
                entity_poly_seq_length=polymer_entity["entity_poly"][
                    "rcsb_sample_sequence_length"
                ],
                entity_poly_entity_type=polymer_entity["entity_poly"]["type"],
                entity_poly_polymer_type=polymer_entity["entity_poly"][
                    "rcsb_entity_polymer_type"
                ],
            )
            for auth_asym_id in polymer_entity[
                "rcsb_polymer_entity_container_identifiers"
            ]["auth_asym_ids"]
        ]
    ...
    def poly_assign_to_asm(self, auth_asym_id: str) -> int:
        ...
        if len(self.asm_maps) == 1:
            ...
            for assembly_map in self.asm_maps:
                ...
                raise LookupError(
                    "Could not assign chain {} to any assembly".format(auth_asym_id)
                )
