...
@click.pass_context
def all(ctx: Context):
    global_status = GlobalView.global_status()
    list(map(lambda a: a.name, list(AssetClass)))
    header = (
        "[RCSB_ID \t" + "".join(["| {}".format(a.name) for a in list(AssetClass)]) + "]"
    )
    click.echo(header)
...
@click.pass_context
@click.argument(
    "assets", required=True, nargs=-1, type=click.Choice(AssetClass._member_names_)
)
...
def assets(
    ctx: Context, assets, reclassify, overwrite, rcsb_sync, all_structs, display
):
    ...
    if rcsb_sync:
        for rcsb_id in GlobalView.status_vs_rcsb():
            print("RCSB Sync: Fetching assets for {}".format(rcsb_id))
            routines = etl_obtain.asset_routines(
                rcsb_id, [AssetClass.profile, AssetClass.mmcif], overwrite
            )
            asyncio.run(etl_obtain.execute_asset_task_pool(routines))
        return

    if all_structs:
        for rcsb_id in GlobalView.list_all_structs():
