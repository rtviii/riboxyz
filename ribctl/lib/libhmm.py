...
def digitize_seq_record(seq_record: SeqRecord, alphabet: Alphabet) -> DigitalSequence:
    ...
    seq_ = pyhmmer.easel.TextSequence(
        name=bytes(seq_record.id, "utf-8"), sequence=seq_record.seq
    )
...
def pick_best_hmm_hit(
    ...
    if len(results) > 1:
        ...
        results = sorted(
            results, key=lambda match_kv: match_kv["match"][0]["score"], reverse=True
        )
        ...
        if best_match == 0 and second_best_match == 0:
            logger.warning(
                "{}.{} : Multiple sensible matches detected, picked {} (smallest e-value) : \n {}".format(
                    chain_info.parent_rcsb_id,
                    chain_info.auth_asym_id,
                    results[0][0],
                    results,
                )
            )
...
def hmm_create(name: str, seqs: Iterator[SeqRecord], alphabet: Alphabet) -> HMM:
    ...
    seq_tuples = [
        TextSequence(name=bytes(seq.id, "utf-8"), sequence=str(seq.seq)) for seq in seqs
    ]
    builder = pyhmmer.plan7.Builder(alphabet)
    background = pyhmmer.plan7.Background(
        alphabet
    )  # ? The null(background) model can be later augmented.
    anonymous_msa = pyhmmer.easel.TextMSA(bytes(name, "utf-8"), sequences=seq_tuples)
    HMM, _profile, _optmized_profile = builder.build_msa(
        anonymous_msa.digitize(alphabet), background
    )
...
def hmm_produce(candidate_class: PolymerClass, organism_taxid: int, seed_sequences: list[SeqRecord], no_cache: bool = False) -> Tuple[PolynucleotideClass, HMM]:  # type: ignore
    ...
    prot_classes = [
        ...
        *list(LifecycleFactors),
        *list(MitochondrialProteinClass),
    ]
    rna_classes = [*list(Polynucleotides)]
...
class PolymerClassFastaRegistry:
    ...
    def __init__(self):
        ...
        for candidate_class in CytosolicProteinClass:
            ...
            self.registry_all_tax_ids[candidate_class] = Fasta(fasta_path).all_taxids(
                "int"
            )

        for candidate_class in MitochondrialProteinClass:
            fasta_path = os.path.join(
                ASSETS["fasta_proteins_mitochondrial"], f"{candidate_class.value}.fasta"
            )
            self.registry_fasta[candidate_class] = Fasta(fasta_path)
            self.registry_all_tax_ids[candidate_class] = Fasta(fasta_path).all_taxids(
                "int"
        ...
        for candidate_class in Polynucleotides:
            fasta_path = os.path.join(
                ASSETS["fasta_rna"], f"{candidate_class.value}.fasta"
            )
            self.registry_fasta[candidate_class] = Fasta(fasta_path)
            self.registry_all_tax_ids[candidate_class] = Fasta(fasta_path).all_taxids(
                "int"
        ...
        for candidate_class in ElongationFactorClass:
            fasta_path = os.path.join(
                ASSETS["fasta_factors_elongation"], f"{candidate_class.value}.fasta"
            )
            self.registry_fasta[candidate_class] = Fasta(fasta_path)
            self.registry_all_tax_ids[candidate_class] = Fasta(fasta_path).all_taxids(
                "int"
        ...
        for candidate_class in InitiationFactorClass:
            fasta_path = os.path.join(
                ASSETS["fasta_factors_initiation"], f"{candidate_class.value}.fasta"
            )
            self.registry_fasta[candidate_class] = Fasta(fasta_path)
            self.registry_all_tax_ids[candidate_class] = Fasta(fasta_path).all_taxids(
                "int"
    ...
    def get_seed_sequences_for_taxid(
        self, taxid: int, max_n_neighbors: int = 5
    ) -> dict[PolymerClass, list[SeqRecord]]:
        _ = {}
...
class PolymerClassesOrganismScanner:
    ...
    def __init__(
        ...
        self.filename_hmms_registry = os.path.join(
            ASSETS["cache_hmm"], "taxid_{}_hmm_scanner".format(organism_taxid)
        )
        self.filename_seed_seqs = os.path.join(
            ASSETS["cache_hmm"], "taxid_{}_seed_seqs".format(organism_taxid)
        )
        ...
        self.seed_sequences = PolymerClassFastaRegistry().get_seed_sequences_for_taxid(
            organism_taxid, max_n_neighbors=max_seed_seqs
        )
    ...
    def classify_seq(
        ...
        hmms = (
            self.hmms_amino
            if alphabet == pyhmmer.easel.Alphabet.amino()
            else self.hmms_rna
        )
    ...
    def info(self) -> dict:
        ...
        _ = {
            "seed_seqs": {
                k: [
                    {"seq": str(seqrecord.seq), "tax_id": seqrecord.id}
                    for seqrecord in v
                ]
                for (k, v) in self.seed_sequences.items()
            },
            "hmms_registry": {
                k: {
                    "name": v.name.decode(),
                    "M": v.M,
                    "nseq": v.nseq,
                    "nseq_effective": v.nseq_effective,
                }
...
class HMMClassifier:
    ...
    @staticmethod
    def pick_best_hit(
        hits: list[dict], bitscore_threshold: float
    ) -> list[PolymerClass]:
        if len(hits) == 0:
            return []
        ...
        return (
            [sorted_by_biscore[0]["class_name"]]
            if sorted_by_biscore[0]["bitscore"] > bitscore_threshold
            else []
        )
    ...
    def classify_chains(self) -> None:
        for chain in self.chains:
            ...
            if organism_taxid not in self.organism_scanners:
                hmmscanner = PolymerClassesOrganismScanner(
                    organism_taxid, max_seed_seqs=5
                )
            ...
            sequence_proper = (
                "".join(
                    list(
                        filter(
                            lambda x: x.lower() in ["u", "c", "g", "a"],
                            list(seq_record.seq),
                        )
                    )
                )
                if chain.entity_poly_polymer_type == "RNA"
                else seq_record.seq
            )
            query_seq = pyhmmer.easel.TextSequence(
                name=bytes(seq_record.id, "utf-8"), sequence=str(sequence_proper)
            ...
            for candidate_class, tophits in cls_hits_tuples:
                for hit in tophits:
                    d_hit = {
                        "fasta_seed": [
                            str(seqrecord.seq)
                            for seqrecord in hmmscanner.seed_sequences[candidate_class]
                        ],
                        "seed_organism_ids": [
                            seqrecord.id
                            for seqrecord in hmmscanner.seed_sequences[candidate_class]
                        ...
                        "consensus": hmmscanner.hmms_registry[
                            candidate_class
                        ].consensus,
                        "domains": [
                            (d.score, d.c_evalue, d.env_from, d.env_to)
