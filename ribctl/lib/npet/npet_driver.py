...
def create_npet_mesh(rcsb_id: str, log_dir: Optional[Path] = None) -> NPETProcessingTracker:
    """
    ...
    try:
        ...
        tracker.add_artifact(ProcessingStage.SETUP, Path(ashapepath))
        ...
        # ENTITY FILTERING
        tracker.begin_stage(ProcessingStage.ENTITY_FILTERING)
        try:
            tunnel_debris = {
                "3J7Z": ["a", "7"],
                "7A5G": ["Y2"],
        ...
        tracker.begin_stage(ProcessingStage.POINT_CLOUD_PROCESSING)
        try:
        ...
        # CLUSTERING
        tracker.begin_stage(ProcessingStage.CLUSTERING)
        try:
        ...
        # REFINEMENT
        tracker.begin_stage(ProcessingStage.REFINEMENT)
        try:
        ...
        # SURFACE EXTRACTION
        tracker.begin_stage(ProcessingStage.SURFACE_EXTRACTION)
        try:
            d3d_alpha = 2
            d3d_tol = 1
            d3d_offset = 2
        ...
        # NORMAL ESTIMATION
        tracker.begin_stage(ProcessingStage.NORMAL_ESTIMATION)
        try:
        ...
        # MESH RECONSTRUCTION
        tracker.begin_stage(ProcessingStage.MESH_RECONSTRUCTION)
        try:
            PR_depth = 6
            PR_ptweight = 3
