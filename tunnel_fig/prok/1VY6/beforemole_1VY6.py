import numpy as np
import os
cmd.load("/Volumes/LaCie/project2/structure/1VY6.cif")
cmd.select('ul4', 'chain BF')
cmd.select('ul22', 'chain BW')
cmd.select('ul23', 'chain BX')
...
cmd.select('residue_l23', 'resi 15 and chain BX')
cmd.create('exit_conserved', 'residue_l23')
coords = cmd.get_coords('residue_l23')
file_path = '/Volumes/LaCie/project2/exit_residue/l23_coords_1VY6.txt'
if os.path.exists(file_path):
   os.remove(file_path)
np.savetxt('/Volumes/LaCie/project2/exit_residue/l23_coords_1VY6.txt', coords[0])
cutoff = 15.0
...
cmd.select("edited", "br. 1VY6 within 80 of last_a")
cmd.create("1VY6_edited", "edited")
cmd.show('spheres', '1VY6_edited')
cmd.save("/Volumes/LaCie/project2/structure/edited/cif/1VY6_edited.cif", "1VY6_edited")
