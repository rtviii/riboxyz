import os
import sys
from loguru import logger

module_dir = os.path.dirname(os.path.abspath(__file__))


updates_logger = logger
updates_logger.remove()
updates_log_path = os.path.join(module_dir, "updates.log")
updates_logger.add(updates_log_path, rotation="10 MB", level="DEBUG", format="{level} {time:YYYY-MM-DD HH:mm:ss} {message}")
updates_logger.add(sys.stdout, level="DEBUG")

access_logger = logger
access_logger.remove()
access_log_path = os.path.join(module_dir, "access.log")
access_logger.add(access_log_path, rotation="10 MB", level="DEBUG")
access_logger.add(sys.stdout, level="DEBUG")