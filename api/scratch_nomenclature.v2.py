...
def msa_dict_cache_tax_pruned_( taxid:int,_:dict):
    dictpath =os.path.join(TAX_PRUNED_MSAS, f"{taxid}.pickle")
...
if __name__ == "__main__":
    ...
    msa_profiles = msa_profiles_dict_prd(phylogenetic_correction_taxid = str(300852))
    eloop        = asyncio.get_event_loop()
    eloop.run_until_complete(
            seq_H_fit_class_multi(chain, msa_profiles, workers=1, vvv=True, omitgaps=False)
        )
