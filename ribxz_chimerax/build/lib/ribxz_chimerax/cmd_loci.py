"""
Command module for 'ribxz loci' functionality.
This command visualizes specific loci (tunnel, PTC, constriction) for a given RCSB ID.
"""
...
BASE_URL = "https://api.ribosome.xyz"
            ...
            with urllib.request.urlopen(url) as response:
                if response.status == 200:
                    ...
                    if 'location' in data:
                        ...
                        # Create a marker with the next available ID
...
def register_loci_command(logger):
    """Register the 'ribxz loci' command with ChimeraX."""
    ...
    register("ribxz loci", desc, loci, logger=logger)